...
//根据收藏/投币比屏蔽 (脚本A: handleBlockedAboveFavoriteCoinRatio)
export const blockVideoFavoriteCoinRatio = (favorite, coin, view, pubdate) => {
    if (!favorite || !coin || !view) return returnTempVal;
    if (!localMKData.isFavoriteCoinRatioBlocking()) return returnTempVal;
    // 播放 < 5000, 收藏 < 50 跳过
    if (view < 5000 || favorite < 50) return returnTempVal;
    // 发布 < 2小时 跳过
    if (pubdate) {
...
export const asyncBlockVideoFavoriteCoinRatio = async (favorite, coin, view, pubdate) => {
    const res = blockVideoFavoriteCoinRatio(favorite, coin, view, pubdate);
    if (res.state) return Promise.reject(res);
};

//根据视频分区屏蔽 (脚本A: handleBlockedVideoPartitions)
export const blockVideoPartition = (partition) => {
    return blockExactAndFuzzyMatching(partition, {
        fuzzyKey: 'videoPartition', fuzzyTypeName: '视频分区(模糊匹配)',
        regexKey: 'videoPartitionCanonical', regexTypeName: '视频分区(正则匹配)'
    });
};

export const asyncBlockVideoPartition = async (partition) => {
